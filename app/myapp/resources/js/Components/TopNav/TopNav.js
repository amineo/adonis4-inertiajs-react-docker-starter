      ...
      <nav css={tw`flex items-center justify-between flex-wrap bg-teal-500 p-6`}>
        ...
        <div css={tw`w-full block flex-grow lg:flex lg:items-center lg:w-auto`}>
          <div css={tw`text-sm lg:flex-grow`}>
            <InertiaLink href="/" css={tw`block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4`}>Home</InertiaLink>
            <InertiaLink href="/about" css={tw`block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white`}>About</InertiaLink>
          </div>
          <div>
            <a href="#" css={tw`inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white hover:border-transparent hover:text-teal-500 hover:bg-white mt-4 lg:mt-0`}>Download</a>
          </div>
        </div>
      </nav>
  );
